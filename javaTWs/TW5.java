...
credentials. The menu options to be provided are for : adding a key-value pair, retrieve
the "value" for a given "key" (first check if the specified key is present),  retrieve
all the keys, retrieve all the values, retrieve all the key-value pairs, change the 
value associated with a key in a HashMap, remove a HashMap element given the key, remove
a HashMap entry with Key and Value, check if a given "value" exists in the Hashmap and
display the HashMap. Read user input where required and display suitable error messages
where applicable.
*/
